...
echo "=============================================="
echo "  VERIFICAÇÃO - DNS E RESOLUÇÃO DE NOMES"
echo "=============================================="
...
# Desafio 1
echo ""
echo "📌 Desafio 1: nslookup"
if [ -f "$PASTA_RESPOSTAS/desafio01.txt" ]; then
    if grep -qi "github.com" "$PASTA_RESPOSTAS/desafio01.txt" && grep -qE "([0-9]{1,3}\.){3}[0-9]{1,3}" "$PASTA_RESPOSTAS/desafio01.txt"; then
...
if [ -f "$PASTA_RESPOSTAS/desafio02.txt" ]; then
    if grep -qi "ufpb.br" "$PASTA_RESPOSTAS/desafio02.txt" && grep -qE "MX|([0-9]{1,3}\.){3}[0-9]{1,3}" "$PASTA_RESPOSTAS/desafio02.txt"; then
...
# Desafio 3
echo ""
echo "📌 Desafio 3: host reverso"
if [ -f "$PASTA_RESPOSTAS/desafio03.txt" ]; then
    if grep -q "8.8.8.8" "$PASTA_RESPOSTAS/desafio03.txt" && grep -qi "dns.google\|domain name\|pointer" "$PASTA_RESPOSTAS/desafio03.txt"; then
...
# Desafio 4
echo ""
echo "📌 Desafio 4: /etc/resolv.conf"
if [ -f "$PASTA_RESPOSTAS/desafio04.txt" ]; then
    if grep -q "nameserver" "$PASTA_RESPOSTAS/desafio04.txt"; then
