...
echo "=============================================="
echo "  VERIFICAÇÃO - FIREWALL E CONTROLE DE TRÁFEGO"
echo "=============================================="
...
# Desafio 1
echo ""
echo "📌 Desafio 1: iptables -L"
if [ -f "$PASTA_RESPOSTAS/desafio01.txt" ]; then
    if grep -qi "Chain\|INPUT\|OUTPUT\|FORWARD\|policy\|target" "$PASTA_RESPOSTAS/desafio01.txt"; then
...
# Desafio 2
echo ""
echo "📌 Desafio 2: ufw status"
if [ -f "$PASTA_RESPOSTAS/desafio02.txt" ]; then
    if grep -qi "Status\|ufw\|active\|inactive\|firewall" "$PASTA_RESPOSTAS/desafio02.txt"; then
...
# Desafio 3
echo ""
echo "📌 Desafio 3: políticas padrão iptables"
if [ -f "$PASTA_RESPOSTAS/desafio03.txt" ]; then
    if grep -qi "Chain\|INPUT\|OUTPUT\|FORWARD\|policy\|ACCEPT\|DROP" "$PASTA_RESPOSTAS/desafio03.txt"; then
...
# Desafio 4
echo ""
echo "📌 Desafio 4: iptables --line-numbers"
if [ -f "$PASTA_RESPOSTAS/desafio04.txt" ]; then
    if grep -qi "Chain\|INPUT\|OUTPUT\|num\|target\|prot" "$PASTA_RESPOSTAS/desafio04.txt"; then
