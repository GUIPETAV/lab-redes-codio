...
echo "=============================================="
echo "  VERIFICAÇÃO - PROXY E VARIÁVEIS DE AMBIENTE"
echo "=============================================="
...
# Desafio 1
echo ""
echo "📌 Desafio 1: variáveis de proxy no ambiente"
if [ -f "$PASTA_RESPOSTAS/desafio01.txt" ]; then
    if grep -qi "proxy\|nenhum" "$PASTA_RESPOSTAS/desafio01.txt"; then
...
# Desafio 2
echo ""
echo "📌 Desafio 2: IP público via curl"
if [ -f "$PASTA_RESPOSTAS/desafio02.txt" ]; then
    if grep -qE "([0-9]{1,3}\.){3}[0-9]{1,3}" "$PASTA_RESPOSTAS/desafio02.txt"; then
...
# Desafio 3
echo ""
echo "📌 Desafio 3: curl sem proxy"
if [ -f "$PASTA_RESPOSTAS/desafio03.txt" ]; then
    if grep -qE "([0-9]{1,3}\.){3}[0-9]{1,3}" "$PASTA_RESPOSTAS/desafio03.txt"; then
...
# Desafio 4
echo ""
echo "📌 Desafio 4: exportar variável de proxy"
if [ -f "$PASTA_RESPOSTAS/desafio04.txt" ]; then
    if grep -qi "proxy\|http_proxy\|https_proxy" "$PASTA_RESPOSTAS/desafio04.txt"; then
